...
class rule:
    ...
    def __init__(self,feature_list,value_list,class_label):
        ...
        self._cover = None
        self._correct_cover = None
        self._incorrect_cover = None
    ...
    def get_cover(self, df, reuse=True):
        if reuse and self._cover is not None:
            return self._cover
        for pattern in self.itemset:
            df = df[df[pattern[0]] == int(pattern[1])]
        if reuse:
            self._cover = df.index.values
        return df.index.values

    def get_correct_cover(self, df, Y, reuse=True):
        if reuse and self._correct_cover is not None:
            return self._correct_cover, self._cover
        ...
        if reuse:
            self._correct_cover = correct_cover
        return correct_cover, indexes_points_covered
    
    def get_incorrect_cover(self, df, Y, reuse=True):
        if reuse and self._incorrect_cover is not None:
            return self._incorrect_cover
        correct_cover, full_cover = self.get_correct_cover(df, Y)
        ret = (sorted(list(set(full_cover) - set(correct_cover))))
        if reuse:
            self._incorrect_cover = ret
        return ret
...
def run_apriori(df, support_thres):
    ...
    for i in range(0,df.shape[0]):
        temp = []
        # active_cols = df.columns.values[df.iloc[i].values == 1]
    ...
    results = []
    for ii,i in enumerate(apriori(dataset, min_support=support_thres)):
        if (ii+1) % 1000 == 0: print(ii)
        results.append(i)
